...
void  insere_chave_arvore(arv* tree, int k)
{
    ...
    else
    {
        ...
        {
            if (chave == NULL)
                insere_chave_no(cria_no(k),folha);
            else
...
void percursoEmNivelOtimizado(arv* t)
{
    noB* aux = t->raiz;
    fila *f = cria_fila();
    no* auxno = NULL;
    ...
    while (!fila_vazia(f))
    {
        ...
        if (aux->esquerda != NULL)
            adicionar((void*)aux->esquerda,f,nivel+1);
        auxno = aux->listaChaves->inicio;
        while (auxno != NULL)
        {
            if (auxno->filho != NULL)
                adicionar(auxno->filho,f,nivel+1);
            auxno = auxno->prox;
        }
    }
}
